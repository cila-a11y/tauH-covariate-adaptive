...
if (length(args) != 1L) {
  stop("Usage: Rscript check_rhDNase_application_outputs.R <RESULTS_DIR>")
}
...
main <- read_csv(file.path(tables, "rhDNase_main_point_estimates.csv"))
main_boot <- read_csv(file.path(tables, "rhDNase_main_estimates_with_bootstrap.csv"))
weights <- read_csv(file.path(tables, "rhDNase_weight_diagnostics_main_H.csv"))
sens_H <- read_csv(file.path(tables, "rhDNase_sensitivity_H.csv"))
sens_lam <- read_csv(file.path(tables, "rhDNase_sensitivity_lambda.csv"))
boot_sum <- read_csv(file.path(boot, "rhDNase_bootstrap_summary.csv"))
